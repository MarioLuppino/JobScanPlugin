...
1. Audit every Markdown file that Claude or a contributor reads, not a JobScan user, and strip formatting that costs tokens and buys nothing: bold runs used for emphasis rather than meaning, decorative headings, tables where a list carries the same content, and sentences that restate the line above them. Done: `jobscan-onboarding/SKILL.md` at `f6c6456`, `job-search/SKILL.md` at `a0c5c0c` (each about 10 percent lighter) and `.claude/skills/release/SKILL.md` at `6bdbec0` (about 6 percent, since it was already less decorated). Those commit messages carry the style the rest of the pass follows. Remaining, heaviest first as measured on `8cc9157`: `docs/ARCHITECTURE.md` (1777 words, 37 bold spans), `plugins/jobscan/skills/jobscan-onboarding/references/intake-questionnaire.md` (1528 / 19), `.claude/skills/handoff/SKILL.md` (1491 / 17), `plugins/jobscan/skills/jobscan-doctor/SKILL.md` (1042 / 16), `references/local-tooling.md` (1161 / 23), `.claude/skills/release/references/release-notes.md` (999 / 16), then the smaller skill and reference files. Behaviour must not change: skill `description` frontmatter and any wording a trigger depends on stay exactly as they are, the 44-question invariant in `.claude/CLAUDE.md` must still pass after `intake-questionnaire.md` is touched, and preflight must exit 0.
...
Verify each file by extracting its backticked identifiers, quoted user-facing phrases and numbers before and after, and diffing those sets. Compare fenced code blocks byte for byte too, and strip them before pairing backticks or the pairing shifts and reports phantom losses. A pass that cannot show that diff clean has not been checked.
...
Continue item 1 on `docs/ARCHITECTURE.md`, applying the style from `f6c6456` and the same before/after verification. It is the one contributor-facing file in the list and is public, so keep it readable as a document rather than compressing it like a skill.
