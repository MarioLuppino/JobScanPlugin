...
- Never screenshot a page to find out what it says, to confirm an action worked, to diagnose a bad read, or to archive a posting. A screenshot costs roughly 15x a page-read tree and is the largest single line in a badly-run scan's bill. Each of those four uses has a cheaper and better tool: read the tree, re-read the tree, `scripts/check-page.mjs`, `scripts/save-posting-pdf.mjs`. A screenshot is for something genuinely visual, and the Process note says which. Full rules, with the measurement: `references/portals.md`, "Screenshots are not a reading tool".
...
0. PRE-DRAFT GATE (every time, hard stop): (a) duplicate-check against `Applied Index.md`; (b) re-load the canonical URL and confirm title + employer + open state. The cheapest way to do (b) is to print it first — `node "${CLAUDE_PLUGIN_ROOT}/scripts/save-posting-pdf.mjs" "<url>" --out "<folder>"` costs no context at all, and a PDF that prints full and passes `node "${CLAUDE_PLUGIN_ROOT}/scripts/check-page.mjs" --pdf description.pdf --expect "<job title>"` has proved the posting live and produced the archive copy in one step. A thin PDF or any verdict other than `ok` means STOP and report; do not create a folder or materials.
1. Next number: highest existing numbered folder + 1 (ignore year folders).
2. Create `<archive>/<N> <Job Title>/`.
3. Populate: `Resume - <role>.docx`, `Cover Letter - <role>.docx`, `Job Posting.md` (downloaded copy), `description.pdf` (the posting as printed by `scripts/save-posting-pdf.mjs` — run it at the gate in step 0, before drafting, because postings close and a packet with no archived description cannot answer "what exactly did they ask for"), `NOTES.txt` (direct apply link + what still needs verifying), and `Outreach Email - <role>.md` only for ideal-fit-but-no-sponsorship cases (draft, never sent).
